repeat wait() until script.Parent.Parent ~= game.ReplicatedStorage
-- Variables
...
if(primary.Orientation.Y ~= 90 and primary.Orientation.Y ~= -90) then
	...
	floorBack.Size = Vector3.new(floorBack.Size.X, 1, floorBack.Size.Z + stairSize)
	
	floorFront:Resize(Enum.NormalId.Right, plot.Position.X + 0.5 * plot.Size.X - (floorFront.Position.X + 0.5 * floorFront.Size.X))
	floorFront.Size = Vector3.new(floorFront.Size.X, 1, floorFront.Size.Z + stairSize)
else
...
--
	...
	floorLeft.Size = Vector3.new(2 + stairSize, 1, floorLeft.Size.Z)
	floorRight.Size = Vector3.new(2 + stairSize, 1, floorRight.Size.Z)
